...
export default function MarketComparisonCell({ productId, ourPrice, comparison }: Props) {
  ...
  return (
    <div ref={containerRef} className="relative flex items-center gap-2 min-w-[160px]">
      ...
      {/* Match Market button — only when priced above market */}
      {isAboveMarket && (
        <button
      ...
      {localError && (
        <span className="text-xs text-red-500" title={localError}>!</span>
      )}
      ...
      {popoverOpen && (
        <div
          className="absolute left-0 top-full mt-1.5 z-50
                     w-60 bg-white border border-slate-200 rounded-xl shadow-xl p-3
                     text-left"
        >
          {/* Header */}
          <p className="text-xs font-semibold text-slate-700 mb-2 flex items-center justify-between">
            <span>
              ...
              {comparison.competitor_count !== 1 ? "s" : ""} nearby
            </span>
            {aboveAvgPct > 0 && (
              <span className="text-red-500 font-bold">
                +{aboveAvgPct.toFixed(0)}% above avg
              </span>
            )}
            {aboveAvgPct < 0 && (
              <span className="text-emerald-600 font-bold">
                {aboveAvgPct.toFixed(0)}% below avg
              </span>
            )}
          </p>
            ...
            {comparison.competitors?.slice(0, 3).map((c, i) => (
              <div key={i} className="flex items-center justify-between text-xs gap-2">
                <div className="flex flex-col min-w-0">
                  ...
                  {c.city && (
                    <span className="text-slate-400 truncate">{c.city}</span>
                  )}
                </div>
                <span
